...
Replacements applied to lessons.json:
  ...
  - Commit hashes (full 40-char SHA-1)         → 000...000
...
def anonymize_lesson(lesson: dict) -> dict:
    out = dict(lesson)
    ...
    if out.get("commit_hash"):
        out["commit_hash"] = ANON_SHA1

    if out.get("task_context"):
        ctx = out["task_context"]
        ctx = SHA1_RE.sub(ANON_SHA1, ctx)
        # Replace any path-like tokens that still contain a personal username
        tokens = ctx.split()
    ...
    with zipfile.ZipFile(src) as zin, zipfile.ZipFile(dst, "w", zipfile.ZIP_DEFLATED) as zout:
        for name in zin.namelist():
            ...
            if name == "lessons.json":
                ...
                lessons = [anonymize_lesson(l) for l in lessons]
    ...
    for lesson in lessons:
        for field in ("folder", "repository", "commit_hash", "task_context"):
            ...
            for match in SHA1_RE.finditer(val):
                if match.group() != ANON_SHA1:
                    issues.append(f"  {lesson['id']}.{field}: SHA-1 hash still present")
    if issues:
